...
def ini_h5_geospatial(fps):
    ...
    for i, fp in enumerate(fps):
        ...
        if tuple(coords_len.values()) == (ds.dims['phony_dim_0'], ds.dims['phony_dim_1']):
            rename = {'phony_dim_0':'time', 'phony_dim_1':'gid'}
        elif tuple(coords_len.values()) == (ds.dims['phony_dim_1'], ds.dims['phony_dim_0']):
            rename = {'phony_dim_0':'gid', 'phony_dim_1':'time'}
        else:
            raise ValueError('Dimensions do not match')
        ds = ds.rename({'phony_dim_0':rename['phony_dim_0'], 'phony_dim_1':rename['phony_dim_1']})
